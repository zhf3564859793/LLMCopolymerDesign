...
The `QSPR_copolymer.ipynb` notebook contains the implementation of the common ML model (RF,XGBoost,SVM,MLP) trained on manually annotated data and LLM-extracted data. This model predicts the glass transition temperature ($T_g$) and melting temperature ($T_m$) of copolymers, enabling quantitative structure-property relationship (QSPR) analysis.
